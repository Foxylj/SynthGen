...
class Encoder(nn.Module):
    def __init__(self, input_channels, hidden_size):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(input_channels, hidden_size, kernel_size=3, stride=1, padding=1),
...
class Decoder(nn.Module):
    def __init__(self, input_channels, hidden_size, output_channels):
        ...
        self.deconv_layers = nn.Sequential(
            nn.ConvTranspose2d(input_channels, hidden_size * 2, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(hidden_size * 2),
            nn.ReLU(),
            nn.ConvTranspose2d(hidden_size * 2, hidden_size, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(hidden_size),
            nn.ReLU(),
            nn.ConvTranspose2d(hidden_size, output_channels, kernel_size=3, stride=1, padding=1)
        )
